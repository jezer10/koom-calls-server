...
describe('SignalingGateway', () => {
  ...
  describe('webrtc:offer / webrtc:answer / webrtc:ice-candidate', () => {
    beforeEach(() => {
      ctx.registry.join('call-1', 'sock-A', 'alice');
      ctx.registry.join('call-1', 'sock-B', 'bob');
    });

    it('forwards webrtc:offer to target user', () => {
      const socket = ctx.makeSocket('sock-A', 'alice');
      const result = ctx.gateway.handleWebrtcOffer(
        { callId: 'call-1', to: 'bob', signal: { sdp: 'x' } },
        socket,
      );
    ...
    it('throws when target is not in the call', () => {
      const socket = ctx.makeSocket('sock-A', 'alice');
      expect(() =>
        ctx.gateway.handleWebrtcOffer(
          { callId: 'call-1', to: 'mallory', signal: {} },
          socket as unknown as Io.Socket,
        ),
      ).toThrow(WsException);
    });

    it('throws when sender is not a participant', () => {
      const socket = ctx.makeSocket('sock-X', 'eve');
      expect(() =>
        ctx.gateway.handleWebrtcOffer(
          { callId: 'call-1', to: 'bob', signal: {} },
          socket as unknown as Io.Socket,
        ),
      ).toThrow(/forbidden/);
    });

    it('handles webrtc:answer and webrtc:ice-candidate similarly', () => {
      const socket = ctx.makeSocket('sock-A', 'alice');
      ctx.gateway.handleWebrtcAnswer(
        { callId: 'call-1', to: 'bob', signal: { sdp: 'y' } },
        socket,
      );
      ctx.gateway.handleWebrtcIceCandidate(
        { callId: 'call-1', to: 'bob', signal: { candidate: 'c' } },
        socket,
      );
      const types = ctx.server.sent
        .map((s) => (s.payload as { type?: string })?.type)
        .filter(Boolean);
      expect(types).toEqual(['webrtc:answer', 'webrtc:ice-candidate']);
    });

    it('throws WsException for invalid webrtc payloads', () => {
      const socket = ctx.makeSocket('sock-A', 'alice');
      expect(() =>
        ctx.gateway.handleWebrtcOffer(
          { callId: '', to: 'bob', signal: {} },
          socket as unknown as Io.Socket,
        ),
