...
describe('RedisPresenceService', () => {
  ...
  it('reports a user as online after markOnline and stores the expected keys', async () => {
    ...
    const keys = (await redis.keys('presence:*')).sort();
    expect(keys).toEqual(
      ['presence:user-socket:user-1:sock-a', 'presence:user:user-1'].sort(),
    );
  });

  it('removes a user from online after markOffline and clears per-socket keys', async () => {
    await service.markOnline('user-1', 'sock-a');
  ...
  it('tracks sockets per call and exposes them via callSockets', async () => {
    ...
    const members = await redis.smembers('presence:call:call-1');
    expect(members.sort()).toEqual(['sock-a', 'sock-b']);
  });
